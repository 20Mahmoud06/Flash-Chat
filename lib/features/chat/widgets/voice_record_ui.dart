          ...
          SnackBar(
            content: CustomText(
              text: friendlyErrorMessage(
                  e, fallback: 'Could not start recording. Please try again.'),
            ),
          ),
        );
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: CustomText(
              text: friendlyErrorMessage(
                  e, fallback: 'Could not pause/continue recording. Please try again.'),
            ),
          ),
        );
      }
    ...
    try {
      ...
      if (file != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: CustomText(text: 'No recording file available to send.'),
            ),
          );
        }
        widget.onCancel();
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: CustomText(
              text: friendlyErrorMessage(
                  e, fallback: 'Could not stop recording. Please try again.'),
            ),
          ),
        );
      }
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          ...
          colors: isDark
              ? [AppColors.voiceGradientEnd, AppColors.voiceGradientStart]
              : const [
                  AppColors.voiceGradientStartLight,
                  AppColors.voiceGradientEndLight
                ],
        ),
                  ...
                  builder: (context, constraints) {
                    ...
                    return Row(
                      ...
                      children: List.generate(numBars, (index) {
                        ...
                        final height = (6.h + 30.h * shape * (0.3 + 0.7 * live))
                            .clamp(4.h, 40.h);
                        return Container(
                          width: barWidth,
                          height: height,
