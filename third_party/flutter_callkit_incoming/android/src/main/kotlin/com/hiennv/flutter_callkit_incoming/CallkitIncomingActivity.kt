...
class CallkitIncomingActivity : Activity() {
    ...
    companion object {
        ...
        /**
         * True when the avatar value is an emoji (or a short string of emoji /
         * decorative characters) rather than an image URL or asset path.
         * Flash Chat stores avatars as emoji, so these are rendered in a
         * TextView instead of being loaded through the image loader.
         */
        fun isEmoji(value: String): Boolean {
            ...
            while (i < trimmed.length) {
                ...
                val isEmojiChar =
                    (codePoint in 0x1F000..0x1FAFF) ||   // emoji & pictographs
                        ...
                        (codePoint >= 0x200D) // ZWJ / skin tones / combining marks
                if (isEmojiChar) hasEmoji = true
    ...
    /// Plays the ringtone/vibration when a NATIVE push launched this activity
    /// (app killed/backgrounded). The plugin path (foreground) already plays
    /// the sound through CallkitNotificationManager, so it never rings twice.
    ...
    private fun incomingData(intent: Intent) {
        ...
        // Flash Chat uses emoji avatars (e.g. "😀"). Render an emoji directly
        // in a TextView instead of trying to load it as an image (which would
        // fall back to the default white circle).
        if (!avatarUrl.isNullOrEmpty() && isEmoji(avatarUrl)) {
            tvAvatarEmoji.visibility = View.VISIBLE
            tvAvatarEmoji.text = avatarUrl
    ...
    private fun onAcceptClick() {
        ...
        // Do NOT dismiss the keyguard here. requestDismissKeyguard() pops the
        // PIN / face-ID / fingerprint prompt, forcing the user to unlock just
        // to join a call they already accepted ("I have to unlock before I can
        // answer"). The call page enables showWhenLocked on the app window
        // (MainActivity.setCallMode via the call pages), so the connected call
        // renders directly above the lock screen — the user can hear, mute and
        // end the call without unlocking. The keyguard stays armed for after
        // the call ends.
        finish()
    }
