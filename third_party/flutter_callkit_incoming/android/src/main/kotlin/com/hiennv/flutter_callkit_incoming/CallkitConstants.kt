...
object CallkitConstants {
    ...
    /// Host-app scoped re-broadcast of an accept/decline decision, fired so the
    /// app can persist the outcome natively (e.g. Firestore) even when the
    /// Flutter engine is not running (app killed) and the event channel is dead.
    const val ACTION_CALLKIT_DECISION =
        "com.hiennv.flutter_callkit_incoming.ACTION_CALLKIT_DECISION"

    const val EXTRA_CALLKIT_DECISION = "EXTRA_CALLKIT_DECISION"
    ...
    /// True when a native FCM/HMS push launched the ring screen. In that case
    /// the ring screen plays its own ringtone/vibration (the plugin's sound
    /// manager is not involved when the app is killed/backgrounded).
    const val EXTRA_CALLKIT_IS_NATIVE_PUSH = "EXTRA_CALLKIT_IS_NATIVE_PUSH"
}
